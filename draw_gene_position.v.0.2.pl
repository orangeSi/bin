#!/usr/bin/perl 
die "perl $0 <list;[sample gff genome seq_id seq_draw_start seq_draw_end] or [sample gff genome]> <prefix> <outdir>\n" if(@ARGV!=3);
...
while(<LI>){
	chomp;
	my ($sample,$gff,$genome,$seq_id,$seq_draw_start,$seq_draw_end)=split(/\s+/,$_);
	...
	while(<GFF>){
		...
		my @arr=split(/\t/,$_);
		if($arr[0] eq $seq_id){
				next unless ($arr[3] >= $seq_draw_start && $arr[4] <= $seq_draw_end);
				print "$arr[0]\t$arr[3]\t$arr[4]\n";
				$genome{$sample}{$arr[0]}{len}=$seq_draw_end -$seq_draw_start+1;
		...
		$gff{$sample}{id}{$arr[0]}{$gene_index}{id}=$gene_id;
		print "id is $gene_id\n";
...
#		print "len is $genome{$sample}{$id}{len};id is $id;sample $sample\n";
		...
		foreach my $index(keys %{$gff{$sample}{id}{$id}}){
			...
			$svg.=&draw_genes($gff{$sample}{id}{$id}{$index}{id},$gff{$sample}{id}{$id}{$index}{start},$gff{$sample}{id}{$id}{$index}{end},$gff{$sample}{id}{$id}{$index}{strand},$gene_height_medium,$gene_height_top,$gene_width_arrow,$shift_x,$top_distance,$sample_single_height,$sample,$id);
...
print SVG "$svg\n</svg>";
close SVG;
`convert $outdir/$prefix.svg $outdir/$prefix.png && echo outfile is $outdir/$prefix.svg`;
...
sub draw_genes(){
	my ($gene_id,$start,$end,$strand,$gene_height_medium,$gene_height_top,$gene_width_arrow,$shift_x,$shift_y,$sample_single_height,$sample,$id)=@_;
	...
	#print "y1 is $y1\n";
	$back="<g style=\"fill: none\"><title>$gene_id,$sample,$id,$start,$end,$strand</title><polygon points=\"$x1,$y1 $x2,$y2 $x3,$y3 $x4,$y4 $x5,$y5 $x6,$y6 $x7,$y7\" style=\"fill:lime;stroke:purple;stroke-width:0\"/> </g>\n";
	return $back;	
