...
=head1 Usage
  ...
  --seq               seq assembly result fa format (need)       
  --cds1              set cds file from Glimmer3 (need)
  --cds2              set cds.gff file from Glimmer3 (need)
  --tRNA1             set tRNA structure file (need)
  --tRNA2             set tRNA gff file (need)
  --rRNA1             set rRNA.fa file from denovo rrnamer (need)
  --rRNA2             set rRNA gff file from denovo (need)
...
my($cds,$cds_gff,$tRNA,$trna_gff,$rRNA,$rRNA_gff,$swiss,$trembl,$nr,$cog,$kegg,$genome);
GetOptions(
	"cds1:s"=>\$cds,
        "cds2:s"=>\$cds_gff,
        "tRNA1:s"=>\$tRNA,
...
`ln -s /home/wangsh3/bin/NCBI_upload/parameter.yaml .`;
`ln -s /home/wangsh3/bin/NCBI_upload/template.sbt .`;
...
`perl /home/wangsh3/bin/NCBI_upload/make_ncbi_tbl.change_Locus.pl --parameter parameter.yaml  --genome $genome --gene cds.gff.filter -annotation Combine_Annotation --swissprot Swissprot.Name -tRNA tRNA.gff.filter -rRNA rRNA.gff.filter --sbt template.sbt`;
close ACDS;
close AAOT;
close CANO;
